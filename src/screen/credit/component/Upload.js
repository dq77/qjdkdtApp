...
class Upload extends PureComponent {
  ...
  pickSingleWithCamera (cropping, mediaType = 'photo') {
    ImagePicker.openCamera({
      ...
      let files = []
      files = files.concat(imageData[this.state.currentCateCode]).concat(image.path)
  ...
  async uploadOCR (photoPath) {
    ...
    if (res.data && res.data.code === '0') {
      ...
      const currentCateCode = this.state.currentCateCode
      let files = []
      files = files.concat(authFiles[this.state.currentCateCode]).concat(res.data.data.key)
            ...
            return (
              <View style={styles.uploadDetail} key={i}>
                ...
                {this.state.imageData[authFileItem.cateCode] && this.state.imageData[authFileItem.cateCode].map((item, index) => {
                  return (
                    <View style={styles.ImgWarp} key={index}>
                      <Touchable onPress={() => { this.showPhoto(true, index, authFileItem.cateCode) }}>
