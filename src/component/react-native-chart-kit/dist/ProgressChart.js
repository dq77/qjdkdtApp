...
import { G, Path, Rect, Svg, Text, Defs, LinearGradient, Stop, Circle, Image } from 'react-native-svg'
...
var ProgressChart = /** @class */ (function (_super) {
  ...
  ProgressChart.prototype.render = function () {
    var _this = this
    var _a = this.props; var width = _a.width; var height = _a.height; var style = _a.style; var data = _a.data; var hideLegend = _a.hideLegend; var strokeWidth = _a.strokeWidth; var radius = _a.radius
    var _b = style.borderRadius; var borderRadius = _b === void 0 ? 0 : _b; var _c = style.margin; var margin = _c === void 0 ? 0 : _c; var _d = style.marginRight; var marginRight = _d === void 0 ? 0 : _d
    var selectStrokeWidth = _a.selectStrokeWidth
    ...
    const aa = pies[0].curves[0].sector.path.print().split(' ')
    // console.log(aa, 'piespies')
    ...
    return (<View style={__assign({
      ...
      <Svg width={width - margin * 2 - marginRight} height={height}>
        ...
        <G x={this.props.width / (hideLegend ? 2 : 2.5)} y={this.props.height / 2} transform={this.props.isRotate ? `translate(0,0)rotate(${data.isRotate})` : ''}>
          <Defs>
            <LinearGradient id="grad1" x1="1" y1="0" x2="0" y2="1">
              <Stop offset="0" stopColor={data.colors[0][0]} stopOpacity="1" />
              <Stop offset="1" stopColor={data.colors[0][1]} stopOpacity="1" />
            </LinearGradient>
          </Defs>
          <Defs>
            <LinearGradient id="grad2" x1="0" y1="0" x2="1" y2="1">
              <Stop offset="0" stopColor={data.colors[1][0]} stopOpacity="1" />
              <Stop offset="1" stopColor={data.colors[1][1]} stopOpacity="1" />
            </LinearGradient>
          </Defs>
          {_this.props.withCustomBarColorFromData && <G>
            {pieBackgrounds.map(function (pie, i) {
              return (<Path key={Math.random()} d={pie.curves[0].sector.path.print()} strokeWidth={strokeWidth} stroke={data.colors[0][0] === 'transparent' ? 'transparent' : 'url(#grad1)'}/>)
            })}
          </G>}
          <G>
            {pies.map(function (pie, i) {
              return (<Path key={Math.random()} strokeLinecap={data.strokeLinecap} strokeLinejoin={data.strokeLinecap} d={pie.curves[0].sector.path.print()} strokeWidth={selectStrokeWidth} stroke={data.colors[1][0] === 'transparent' ? 'transparent' : 'url(#grad2)'}/>)
            })}
            {_this.props.hideCircle && <Circle cx={0} cy={-dp(100)} r={dp(5)} fill="white" />}
            {_this.props.hideCircle && <Circle cx={aa[9]} cy={ aa[10]} r={dp(5)} fill="white" />}
            {/* 47.552826 -15.45085 */}
          </G>
          {_this.props.isShowText && <Text key={Math.random()} fill="#91969A" fontSize={_this.props.textTopFont} textAnchor="middle" y={-dp(16)} >
            {data.textTop}
          </Text>}
          {_this.props.isShowText && <Text key={Math.random()} fill={data.colors[1][1]} fontSize={_this.props.textBottomFont} textAnchor="middle" y={dp(30)}>
            {data.textBottom}
          </Text>}
          {_this.props.isShowOneText && <Text transform={this.props.isRotate ? `translate(0,0)rotate(${-data.isRotate})` : ''} x={0} y={dp(10)} key={Math.random()} fill={data.showOneTextColor} fontSize={data.showOneTextFont} textAnchor="middle">
            {data.showOneText}
          </Text>}
          {_this.props.hidePointer && <Image
            transform={`translate(0,0)rotate(${data.data * 360})`}
            x={-50}
            y={-50}
            width="100"
            height="100"
            href={require('./zhizheng.png')}
          >
          </Image>}
          {/* {legend} */}
        </G>
